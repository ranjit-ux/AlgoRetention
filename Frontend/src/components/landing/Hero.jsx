...
const Hero = () => {
  ...
  useEffect(() => {
    ...
    // Mini heatmap
    const hw = [
      0, 1, 2, 0, 1, 3, 0, 2, 1, 0, 3, 2, 1, 0, 2, 1, 3, 0, 2, 1, 0, 1, 2, 3, 0,
      2, 1, 0, 2, 3, 1, 0, 1, 2, 0, 3, 2, 1, 0, 2,
    ];
    if (hmapRef.current) {
      for (let i = 0; i < 56; i++) {
  ...
  return (
    <section
      id="hero"
      className="relative min-h-screen flex flex-col items-center justify-center pt-28 pb-20 px-[6vw] text-center overflow-hidden bg-background text-foreground"
    >   
      {/* Glow orbs */}
      <div
        className="absolute top-[-120px] right-[-120px] w-[700px] h-[700px] rounded-full pointer-events-none"
        style={{
          background:
            "radial-gradient(circle,rgba(217,119,87,0.13) 0%,transparent 68%)",
        }}
      />
      <div
        className="absolute bottom-[-60px] left-[-100px] w-[500px] h-[500px] rounded-full pointer-events-none"
        style={{
          background:
            "radial-gradient(circle,rgba(217,119,87,0.09) 0%,transparent 68%)",
        }}
      />
      ...
      <h1 className="font-serif text-[clamp(3rem,6.5vw,5.6rem)] leading-[1.06] tracking-tight mb-6 animate-fadeUp [animation-delay:100ms]">
        Solve once.
        <br />
        <em className="text-[#D97757] not-italic font-serif italic">
          Remember forever.
        </em>
      </h1>
      ...
      <p className="text-[clamp(.95rem,1.6vw,1.1rem)] text-[#7A736A] max-w-[520px] mx-auto mb-10 font-light leading-[1.75] animate-fadeUp [animation-delay:200ms]">
        Most programmers forget 80% of problems within a week. AlgoRetention
        schedules intelligent reviews using SM-2 so your hard-earned patterns
        never fade before the interview.
      </p>
  ...
  See how it works →
      ...
      <div className="mt-10 flex items-center gap-3 justify-center animate-fadeUp [animation-delay:450ms]">
        ...
        <p className="text-[12.5px] text-[#7A736A]">
          Trusted by{" "}
          <strong className="text-foreground">1,200+ developers</strong>{" "}
          preparing for top-tier placements
        </p>
      </div>
      ...
      <div className="mt-14 w-full max-w-[840px] rounded-[20px] overflow-hidden border border-[#E4DDD5] shadow-[0_40px_80px_rgba(0,0,0,0.09),0_0_0_1px_rgba(255,255,255,0.5)_inset] animate-fadeUp [animation-delay:550ms]">
        ...
        <div className="bg-white p-5 grid grid-cols-4 gap-2.5">
          {[
            { label: "Problems Solved", val: "142", color: "text-black" },
            { label: "Retention Score", val: "94%", color: "text-[#1A9970]" },
            { label: "Current Streak", val: "18🔥", color: "text-[#D97757]" },
            { label: "Due Today", val: "3", color: "text-black" },
          ].map((s) => (
            <div key={s.label} className="bg-[#F8F4EF] rounded-xl p-3">
              <div className="text-[10px] text-black font-medium mb-1">
                {s.label}
              </div>
              <div className={`text-[1.35rem] font-bold ${s.color}`}>
                {s.val}
              </div>
            </div>
          ))}
        ...
        <div className="bg-white px-5 pb-5 grid grid-cols-[1fr_200px] gap-2.5">
          <div className="bg-[#F8F4EF] rounded-xl p-3.5 h-[110px]">
            <div className="text-[10px] font-medium text-[#7A736A] mb-2">
              Retention trend 14 days
            </div>
